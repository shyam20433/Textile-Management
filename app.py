...
@app.route('/place_order/<dress_id>', methods=['GET', 'POST'])
def place_order(dress_id):
    dress = dresses_collection.find_one({"_id": ObjectId(dress_id)})
    if not dress:
        flash("Dress not found.")
        return redirect(url_for('index'))
    ...
    if request.method == 'POST':
        try:
            ...
            # Store order data in the session temporarily
            session['order'] = {
                "dress_id": dress_id,
                "gender":gender,
                "dress_type": dress_type,
            ...
            flash(f"Order placed successfully for {quantity} of Dress Type: {dress_type}, Size: {size}")
            return redirect(url_for('order_success'))  # Redirect to the order success page
        except Exception as e:
            flash(f"Error placing the order: {e}")
            return redirect(url_for('dress_detail', dress_id=dress_id))

    else:
        return render_template('order_confirmation.html', dress=dress)
...
def verify_otp():
    if request.method == 'POST':
        ...
        session['otp'] = otp  # Store OTP in session for demonstration purposes
...
def order_customization(dress_id):
    ...
    if request.method == 'POST':
        ...
        # Save the customized order in dress_types_collection
        dress_types_collection.insert_one({
            "dress_id": ObjectId(dress_id),
            "gender": gender,
            "dress_design":dress_design,
        ...
        flash(f"Order for {quantity} {dress_type}(s) in {size} size has been placed successfully!")
...
def review_order(dress_id):
    # Fetch dress from the dress collection
    dress = dresses_collection.find_one({"_id": ObjectId(dress_id)})
    if not dress:
        flash("Dress not found.")
        return redirect(url_for('index'))
    ...
    if request.method == 'POST':
        # If the user is submitting customization details
        gender = request.form.get('gender')
        dress_type = request.form.get('dress_type')
        dress_design = request.form.get('dress_design')
        size = request.form.get('size')
        ...
        # Check conditions and assign the image URL
        if gender == 'baby' and dress_type in ('Footed romper', 'Snap-up romper'):
            color = request.form.get('color')
            image_url = image_url_map_baby.get(dress_design, {}).get(color)
        elif gender == 'baby' and dress_type == 'Hoody':
            color = request.form.get('color')
            image_url = image_url_map_baby.get(dress_design, {}).get(color)
        elif gender == 'baby' and dress_type == 'Cotton jumpsuit':
            color = request.form.get('color')
            image_url = image_url_map_baby.get(dress_design, {}).get(color)
        elif gender == 'men' and dress_type == 'Shirt':
            color = request.form.get('color')
            image_url = image_url_map_men.get(dress_design, {}).get(color)
        elif gender =='men' and dress_type == 'Jacket':
            color = request.form.get('color')
            image_url = image_url_map_men.get(dress_design, {}).get(color)
        elif gender == 'women' and dress_type == 'Jacket':
            color = request.form.get('color')
            image_url = image_url_map_women.get(dress_design, {}).get(color)
        elif gender == 'women' and dress_type == 'Pant':
            color = request.form.get('color')
            image_url = image_url_map_women.get(dress_design, {}).get(color)


        else:
            image_url = '/static/dress/red ft.jpg'  # Default for black

        # Store the order data and the image URL in the session
        session['order'] = {
            "color":color,
            "dress_id": dress_id,
            "gender": gender,
    ...
    # Render the review_order.html template, passing the order details and the image URL
    return render_template('review_order.html', dress=dress, gender=order['gender'],
                           dress_type=order['dress_type'], dress_design=order['dress_design'],
...
def order_success():
    ...
    user = login_collection.find_one({"email": user_email})

    if not user:
        flash("User not found. Please log in.")
        return redirect(url_for('login'))

    # Fetch the dress details from the dress_types collection using the order's dress_id
    dress_details = dress_types_collection.find_one({"dress_id": ObjectId(order['dress_id'])})

    if not dress_details:
        flash("Dress details not found.")
        return redirect(url_for('index'))

    # Render the order success page with updated order details
